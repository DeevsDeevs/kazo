...
async def test_text_items_without_prices_saved():
    ...
    exp = _make_expense(items_json=json.dumps(items), amount=12.30, amount_base=12.30)
    expense_id = await save_expense(exp)

    saved_items = await get_expense_items(expense_id)
    assert len(saved_items) == 3
    assert all(i["price"] is None for i in saved_items)
...
async def test_text_items_with_quantity():
    ...
    exp = _make_expense(items_json=json.dumps(items), amount=6.50, amount_base=6.50)
    expense_id = await save_expense(exp)
...
async def test_items_json_stored_on_expense():
    ...
    exp = _make_expense(items_json=items_json, amount=4.50, amount_base=4.50)
    expense_id = await save_expense(exp)
...
async def test_mixed_items_some_with_prices():
    ...
    exp = _make_expense(items_json=json.dumps(items), amount=8.00, amount_base=8.00)
    expense_id = await save_expense(exp)
    ...
    assert "Unknown item" in names
    assert "Cheese" in names
    null_item = next(i for i in saved_items if i["name"] == "Unknown item")
