...
async def _add_expense(chat_id, amount_eur, category, expense_date):
    db = await get_db()
    await db.execute(
        ...
        (chat_id, amount_eur, amount_eur, category, expense_date),
    )
    await db.commit()
...
async def test_set_and_get_budget():
    ...
    assert b.amount_eur == 2000.0
    assert b.category is None

    got = await get_budget(CHAT)
    assert got is not None
    assert got.amount_eur == 2000.0
