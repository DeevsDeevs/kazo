...
async def test_date_filter():
    ...
    assert len(rows) == 1
    assert rows[0]["amount_base"] == 25.0
...
async def test_delete_last_expense():
    ...
    assert deleted["amount_base"] == 20.0

    rows = await get_expenses(chat_id=1)
    assert len(rows) == 1
    assert rows[0]["amount_base"] == 10.0
...
async def test_detect_recurring_triggers():
    today = date.today()
    for _i in range(2):
        await save_expense(_make_expense(store="Netflix", amount=15.99, amount_base=15.99, expense_date=today))
    assert await detect_recurring(1, "Netflix", 15.99) is True


async def test_detect_recurring_within_tolerance():
    today = date.today()
    await save_expense(_make_expense(store="Spotify", amount=9.99, amount_base=9.99, expense_date=today))
    await save_expense(_make_expense(store="Spotify", amount=10.50, amount_base=10.50, expense_date=today))
    assert await detect_recurring(1, "Spotify", 10.00) is True


async def test_detect_recurring_not_enough():
    today = date.today()
    await save_expense(_make_expense(store="OneTime", amount=50.0, amount_base=50.0, expense_date=today))
